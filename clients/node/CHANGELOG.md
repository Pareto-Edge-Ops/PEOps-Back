...
Versions are kept in **lockstep** with the Python `peops-sdk` so the dashboard's
single `GET /api/sdk/version` describes both clients truthfully.

## 0.2.0 — 2026-06-29

- Initial Node release — API parity with the Python `peops-sdk` 0.2.0.
- **No base URL in your code**: `baseUrl` is optional everywhere — it defaults to
  the hosted PEOps origin (override with `PEOPS_BASE_URL` or the `baseUrl`
  option). SDK code only needs the deployment id + API key. `PeopsClient` is now
  `new PeopsClient(deploymentId, apiKey, { baseUrl? })`.
- **Hosted inference**: `PeopsClient.infer()` against
...
- **Local serving**: `LocalRunner.fromDeployment()` pulls the deployed,
  PEOps-compressed artifact (sha256-cached on disk) and runs it with
...
- **Built-in telemetry**: every local inference is measured (latency breakdown,
  ...
  the PEOps dashboard, plus periodic system snapshots and windowed input/output
  distribution stats that power prediction/input drift alerts. Opt out with
  `reportTelemetry: false` or `PEOPS_SDK_TELEMETRY=0`.
